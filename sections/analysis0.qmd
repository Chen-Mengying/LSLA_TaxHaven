...
desag |>
  filter(is_tax_haven == TRUE) |>
  distinct(bvd_id_number, country_iso2) |>   # 🔥 unique investor × country
...
desag |>
  filter(is_tax_haven == TRUE) |>
  distinct(deal_id, target_country_code_alpha2) |>  # 🔥 deal level
...
```{r}
#| label: fig-taxhaven-by-sector-stackedbar
#| fig-cap: "Tax Haven Involvement by Sector (Proportional Stacked Bar)"
...
```{r}
#| label: fig-top10-taxhaven-investor-by-sectot
#| fig-cap: "Sector Composition of Top 10 Tax Haven Investor Countries"

# Step 1: Compute counts by (tax haven investor country × sector)
...
```{r}
#| label: fig-Global Investment Flows
#| fig-cap: "Global Investment Flows from Tax Havens to Target Countries"

# ============================
# 1. Build bilateral flow table
# ============================

flows <- desag |>
  filter(is_tax_haven == TRUE) |>
  # origin = investor country
  # destination = target country
  count(
    origin = country_iso2,
    destination = target_country_code_alpha2,
    name = "n"
  ) |>
  filter(!is.na(origin), !is.na(destination), origin != "", destination != "")

# ============================
# 2. Get coordinates for origin/destination
# ============================

# Extract the centroid (lat/lon) of each country
country_coords <- countries_sf_main |>
  st_centroid() |>
  mutate(
...
# Merge coords for origins
flow_map <- flows |>
  left_join(country_coords, by = c("origin" = "ISO")) |>
  rename(
    lon_o = lon,
    lat_o = lat
  ) |>
  left_join(country_coords, by = c("destination" = "ISO")) |>
  rename(
    lon_d = lon,
    lat_d = lat
  ) |>
...
world_map <- countries_sf_all

# ============================
# 4. Plot: global investment flows
# ============================
...
ggplot() +
  ...
  coord_sf() +
  labs(
    title = "Global Investment Flows from Tax Havens to Target Countries",
    subtitle = "Each curved line represents investment involvement from a tax haven investor country",
    caption = "Data: Land Matrix + Orbis"
  ) +
  theme_minimal() +
  theme(
...
flows <- desag |>
  filter(is_tax_haven == TRUE) |>
  filter(!is.na(country_iso2), !is.na(target_country_code_alpha2)) |>
  # rename for clarity
  transmute(
    origin = country_iso2,
    destination = target_country_code_alpha2,
    year = initiation_year,
    sector = intention_investment_cat,
    surface = current_contract_size
  ) |>
...
flows <- flows |> filter(period != "Unknown")

period_order <- c("Before 2000", "2000–2006", "2007–2015", "2016–2023")
flows$period <- factor(flows$period, levels = period_order)
```

```{r}
coords <- countries_sf_main |>
  st_centroid() |>
  mutate(
    lon = st_coordinates(geometry)[,1],
    lat = st_coordinates(geometry)[,2]
...
flow_map <- flows |>
  left_join(coords, by = c("origin" = "ISO")) |>
  rename(lon_o = lon, lat_o = lat) |>
  left_join(coords, by = c("destination" = "ISO")) |>
  rename(lon_d = lon, lat_d = lat) |>
  drop_na(lon_o, lat_o, lon_d, lat_d) |>
  filter(!(lon_o == lon_d & lat_o == lat_d))

nodes_df <- flows |>
  group_by(destination, sector, period) |>
  summarise(
...
nodes_df$period <- factor(nodes_df$period, levels = period_order)
```
...
ggplot() +
  ...
  geom_curve(
    ...
    aes(
      x = lon_o, y = lat_o,
      xend = lon_d, yend = lat_d,
      group = interaction(origin, destination)
    ),
    curvature = 0.15,
    color = "grey40",
    alpha = 0.20,
  ...
  scale_size_continuous(
    range = c(1.2, 7),      # 适合地图比例
    name = "Cumulated surface (ha)"
  ) +
  scale_fill_manual(
    values = colorspace::qualitative_hcl(
      length(unique(nodes_df$sector)), 
      palette = "Dark3"
    ),
    name = "Main investment type"
  ) +
  ...
  labs(
    title = "Global Investment Flows from Tax Havens to Target Countries",
    subtitle = "Investment pathways by period",
    x = NULL,
    y = NULL
  ) +
